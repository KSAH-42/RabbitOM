...
And most of C++ forked repo use an Authenticator capable to computeDigestResponse, an try to createAuthenticatorString but it doesn't support latest digest algorithms, and for instance hitachie/thalezed products with their great picatchou recorder (that will probably never exist without this lib) that embedded this dll just fails. I hope that one day some sales engineer will not pushed some security camera to trigger a vms product replacement silently.
