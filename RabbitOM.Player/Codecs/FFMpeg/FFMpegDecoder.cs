...
namespace RabbitOM.Player.Codecs.FFMpeg
{
    ...
    public unsafe sealed class FFMpegDecoder : Decoder
    {
        ...
        public override void Decode( byte[] buffer )
        {
            ...
            {
                // normally, in this case, it's highly recommended to set to default _rawPacket->data as null after unpin/unfixed the buffer adress and calling ffmpeg.decode func using a try finally bloc, the reason come from that the compactor on the GC can change the address of the raw buffer
                ...
                // so the right approach is to force a clear on these members after...
