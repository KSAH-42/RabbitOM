...
namespace RabbitOM.Net.Rtps.Codecs
{
	...
	/// </summary>
	public sealed class H265Codec : VideoCodec
	{
		...
		public H265Codec(byte[] sps_pps, byte[] vps)
		{
		...
		public static H265Codec Create()
		{
		...
		/// <returns>returns an instance, otherwise null</returns>
		public static H265Codec Create(string sps, string pps, string vps)
		{
			...
			try
			{
				...
				IEnumerable<byte> spsBytes = CodecConstants.H265StartMarker.Concat(spsBuffer);
				if (spsBytes == null)
				{
					return null;
				}
				...
				IEnumerable<byte> ppsBytes = CodecConstants.H265StartMarker.Concat(ppsBuffer);
				if (ppsBytes == null)
				{
					return null;
				}
				return new H265Codec(spsBytes.Concat(ppsBytes).ToArray(), Convert.FromBase64String(vps));
			}
			catch (Exception)
			{
			}
