...
{
    using RabbitOM.Streaming.Experimentation.Rtsp2BeRemoved;
    using RabbitOM.Streaming.Experimentation.Rtsp2BeRemoved.Headers;
    
    [TestFixture]
    public class PublicRtspHeaderValueTest
    {
        [TestCase( "OPTIONS" )]
        [TestCase( " OPTIONS " )]
        [TestCase( " , OPTIONS , " )]
        ...
        public void CheckRemovingMethods()
        {
            ...
            Assert.IsTrue( header.RemoveMethodBy( x => x == RtspMethod.RECORD ) );
            Assert.IsFalse( header.RemoveMethodBy( x => x == RtspMethod.RECORD ) );
            Assert.AreEqual( 1 , header.Methods.Count );
