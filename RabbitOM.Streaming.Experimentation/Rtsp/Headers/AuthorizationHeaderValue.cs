...
namespace RabbitOM.Streaming.Experimentation.Rtsp.Headers
{
    ...
    public sealed class AuthorizationHeaderValue
    {
        ...
        public static bool TryParse( string input , out AuthorizationHeaderValue result )
        {
            result = null;

            if ( HeaderParser.TryParse( input , " " , out string[] tokens ) )
            {
                var header = new AuthorizationHeaderValue() { Scheme = tokens.FirstOrDefault() };
                ...
                if ( HeaderParser.TryParse( string.Join( " " , tokens.Skip( 1 ) ) , "," , out tokens ) )
                {
                    ...
                    if ( HeaderProtocolValidator.IsValidToken( header.Scheme ) && HeaderProtocolValidator.IsValidToken( header.UserName ) )
                    {
                        result = header;
                    }
                }
        ...
        public override string ToString()
        {
            ...
            {
                if ( HeaderParser.TryParse( extension , "=" , out KeyValuePair<string,string> parameter ) )
                {
