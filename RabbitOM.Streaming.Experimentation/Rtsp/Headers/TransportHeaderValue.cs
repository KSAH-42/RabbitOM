...
namespace RabbitOM.Streaming.Experimentation.Rtsp.Headers
{
    ...
    public sealed class TransportHeaderValue
    {
        ...
        public static bool TryParse( string input , out TransportHeaderValue result )
        {
            result = null;

            if ( HeaderParser.TryParse( input , ";" , out string[] tokens ) )
            {
                var header = new TransportHeaderValue();
                ...
                foreach ( var token in tokens )
                {
                    if ( HeaderParser.TryParse( token , "=" , out KeyValuePair<string,string> parameter ) )
                    {
                        ...
                        else if ( HeaderProtocolValidator.IsValidTransport( parameter.Key ) )
                        {
                            header.Transport = token;
                        }
                        else if ( HeaderProtocolValidator.IsValidTransmission( parameter.Key ) )
                        {
                    ...
                    {
                        if ( HeaderProtocolValidator.IsValidTransport( token ) )
                        {
                            header.Transport = token;
                        }
                        else if ( HeaderProtocolValidator.IsValidTransmission( token ) )
                        {
                            header.Transmission = token;
                        }
                        else    
                ...
                if ( HeaderProtocolValidator.IsValidToken( header.Transport ) && HeaderProtocolValidator.IsValidToken( header.Transmission ) )
                {
                    result = header;
                }
            }
        ...
        {
            if ( HeaderProtocolValidator.IsValid( value = ValueNormalizer.Normalize( value ) ) )
            {
