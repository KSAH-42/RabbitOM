...
namespace RabbitOM.Net.Rtsp.Remoting
{
    ...
    public sealed class RTSPProxyRequestManager : IDisposable
    {
        ...
        private readonly RTSPProxyMessageDecoder      _decoder               = null;
        ...
        public RTSPProxyRequestManager( RTSPProxy proxy )
        {
            ...
            _decoder = new RTSPProxyMessageDecoder();
        ...
        public bool SendRequest( RTSPMessageRequest request , out RTSPMessageResponse response )
        {
            ...
            try
            {
                ...
                if ( ! _requestHandlers.Add( handler ) )
                {
                    return false;
                }
        ...
        private void ListenMessages()
        {
            try
            {
                ...
                _chunks.Enqueue( new RTSPChunk( data ) );
            }
            catch ( Exception ex )
            {
                OnError( ex );
        ...
        private void ReceiveChunks()
        {
            while ( _chunks.Any() )
            {
                if ( _chunks.Dequeue( out RTSPChunk chunk ) )
                {
                    DecodeChunk( chunk );
                }
            }
        ...
        private void DecodeChunk( RTSPChunk chunk )
        {
            ...
            _decoder.Write( chunk.Data , 0 , chunk.Data.Length );
        ...
        private void OnDataReceived( RTSPPacket packet )
        {
            ...
            _proxy.DispatchEvent( new RTSPConnectionDataReceivedEventArgs( packet ) );
        }
