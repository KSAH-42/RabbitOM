...
namespace RabbitOM.Streaming.RtspV2.Transports
{
    ...
    public sealed class RtspStream : IStream
    {
        ...
        public int Peek()
        {
            ...
            // TODO: /!\ why using this statement here ? just get out the conditional statement, add a tone of units tests to make sure that the statement below will never comes
        ...
        public void Flush()
        {
            try
            {
                ...
                while ( offset < _writeBuffer.Length )
                {
                    var bytesToSend = _outputStream.Read( _writeBuffer , offset , _writeBuffer.Length );
                    ...
                    _transport.Send( _writeBuffer , offset , bytesToSend );
